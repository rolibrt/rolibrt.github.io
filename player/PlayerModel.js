...
function createSkinnedBox(width, height, depth, texture, uvMap) {
  ...
  for (let face = 0; face < 6; face++) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const x = i === 0 || i === 3 ? uMin : uMax;
      const y = i === 0 || i === 1 ? vMax : vMin;
...
const UVMaps = {
  head: [ [8,8,16,16], [0,8,8,16], [8,0,16,8], [16,0,24,8], [16,8,24,16], [24,8,32,16] ],
...
export function createMinecraftPlayer(skinTextureUrl) {
  ...
  const texture = new THREE.TextureLoader().load(skinTextureUrl, () => {
  ...
  group.position.y = 0.9; // Adjusted to keep feet at y = 0 (1.8 * 16 * 0.5)
